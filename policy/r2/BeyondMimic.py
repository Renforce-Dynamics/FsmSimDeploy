...
class BeyondMimic(FSMState):
    def __init__(self, state_cmd:StateAndCmd, policy_output:PolicyOutput):
        ...
        config_path = os.path.join(current_dir, "config", "r2.yaml")
        with open(config_path, "r") as f:
            ...
            self.ref_body_pos_w = np.zeros((1, 11, 3), dtype=np.float32)
            self.ref_body_quat_w = np.zeros((1, 11, 4), dtype=np.float32)
            self.ref_body_lin_vel_w = np.zeros((1, 11, 3), dtype=np.float32)
            self.ref_body_ang_vel_w = np.zeros((1, 11, 3), dtype=np.float32)
            # load policy
    ...
    def enter(self):
        ...
        # self.action = np.zeros(self.num_actions)
        print("entered beyond mimic policy")
    ...
    def run(self):
        ...
        motion_anchor_ori_b = self.matrix_from_quat(robot_quat).T @ self.matrix_from_quat(ref_anchor_ori_w)

        ang_vel = self.state_cmd.ang_vel
        
        dqj = self.state_cmd.dq
        
        mimic_obs_buf = np.concatenate((self.ref_joint_pos.squeeze(),
                                        self.ref_joint_vel.squeeze(),
        ...
        # if self.counter_step < 100:
        #     self.res.append(mimic_obs_buf.reshape(1, -1))
        # elif self.counter_step == 100:
        #     self.res = np.concatenate(self.res, 0)
        #     np.save("./obs.npz", self.res)
        #     print("saved obs.npy")
